...
def process_clip(image):
    ...
    #mag_binary = mag_thresh(image_, sobel_kernel=ksize, mag_thresh=(20, 250))
    #dir_binary = dir_threshold(image_, sobel_kernel=ksize, thresh=(0.6, 1.3))

    #select s channel
    c_binary, mask_w, mask_y = imgselect(image_, sthresh=(90, 255),vthresh=(50,255))

    #print (sum(mask_w),sum(mask_y))


    processed_img = np.zeros_like(gradx)
    #combined[((gradx == 1) & (grady == 1))| ((dir_binary == 1)&(dir_binary == 1))] = 1
    ...
    #print(len(curve_centers.recent_centers))
    ...
    #display the info 
    cv2.putText(road_combined,'Radius of Curvature = '+str(round(right_curverad,3))+'(m)',(50,50), cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
...
curve_centers = tracker(window_width, window_height,margin,ym = 10/728, xm = 4/667, smooth_factor = 12)
video_clip = clip1.fl_image(process_clip)
